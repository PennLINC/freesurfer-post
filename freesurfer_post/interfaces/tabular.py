...
class SummarizeRegionStats(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        for hemi in hemispheres:
            ...
            gwpct_df_ = statsfile_to_df(gwr_stats_file, hemi, atlas, column_suffix="_wgpct")
        ...
        session_id = None if not isdefined(self.inputs.session_id) else self.inputs.session_id
        ...
        out_df.to_csv(output_dir / f"{output_prefix}_atlas-{atlas}_surfacestats.tsv", sep="\t", index=False)
        return runtime
...
class FSStats(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        subject_id = self.inputs.subject_id
        session_id = self.inputs.session_id if isdefined(self.inputs.session_id) else None
